...
import {
  EleventyCollection,
  EleventyLayout,
} from "@henrycatalinismith/11tytype";
...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addCollection("mixins", function (collectionApi) {
    return collectionApi
      ...
      .map((f) => {
        f.data.dependencies = (f.data.dependencies || []).map((name) => {
          return collectionApi.getFilteredByGlob(`mixins/${name}.scss`)[0];
        });
        return f;
      })
      .map((f) => {
  ...
  eleventyConfig.addCollection("readmeSections", function () {
    ...
    for (const section of readmeDocument.window.document.querySelectorAll(
      "section",
    )) {
      ...
      [
        ...
        (url) => {
          if (url.href.startsWith("/") && process.env.GITHUB_ACTIONS) {
            return `https://henrycatalinismith.codeberg.page/picknmix/${url.href}`;
          }
        },
      ],
    ],
